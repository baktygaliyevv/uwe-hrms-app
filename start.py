...
class MainWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        for F in (LoginFrame, ManageUsersFrame, ManageRestaurantsFrame, ManageTablesFrame, ManagePromocodesFrame, ManageProductsListFrame, ManageMenuFrame):
            frame = F(container, self)
            self.frames[F.__name__] = frame
            frame.grid(row = 0, column = 0, sticky = "nsew")

        # self.show_frame('LoginFrame') # TODO return when MainMenu is done
        self.show_frame('ManageMenuFrame')
